...
generate_calendar() {
    ...
    for ((d=1; d<=ndim; d++)); do
        if [[ $month -eq $CURRENT_MONTH && $year -eq $CURRENT_YEAR && $d -eq $TODAY ]]; then
            # Highlight today's date with a yellow background and dark gray text
...
ROFIPID=$!
sleep 1.0
maim /home/dionovan/.gemini/antigravity-cli/brain/54252fb6-f822-4649-80b6-70c42fc9367e/test_calendar_markup.png
kill $ROFIPID
pkill -f rofi
